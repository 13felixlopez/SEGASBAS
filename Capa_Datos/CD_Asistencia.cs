...
namespace Capa_Datos
{
    public class CD_Asistencia
    {
        ...
        {
            List<CE_Asistencia> lista = new List<CE_Asistencia>();
            totalRegistros = 0;
            try
            {
                using (SqlConnection oConexion = CD_Conexion.ObtenerConexion())
                {
                    SqlCommand cmd = new SqlCommand("sp_ObtenerAsistenciaConPaginado", oConexion);
        ...
        public bool InsertarAsistencia(CE_Asistencia obj, out string Mensaje)
        {
            ...
            try
            {
                using (SqlConnection oConexion = CD_Conexion.ObtenerConexion())
                {
                    SqlCommand cmd = new SqlCommand("sp_InsertarAsistencia", oConexion);
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        public List<CE_Asistencia> BuscarAsistencias(string terminoBusqueda)
        {
            ...
            try
            {
                using (SqlConnection oConexion = CD_Conexion.ObtenerConexion())
                {
                    SqlCommand cmd = new SqlCommand("sp_BuscarAsistencia", oConexion);
                    cmd.CommandType = CommandType.StoredProcedure;
